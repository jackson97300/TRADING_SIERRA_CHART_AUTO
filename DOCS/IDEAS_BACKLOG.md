...
- [2026-09-09] Bloc A review R2/R3 (non bloquants) : (a) kill dur (SIGKILL/coupure) pendant un coureur du soir laisse un fichier vide ou partiel malgre R8 — fix propre = ecrire en .tmp puis os.replace (reactions/marges font deja un write final unique, quasi gratuit) ; (b) marges compte les episodes sur OR(short,long), le journal officiel PAR COTE — un long qui demarre pendant un short vrai compte 1 vs 2 (commente dans marges.py, a savoir en lecture jour 61).
- [2026-09-09] RESERVE FABLE (verification b46f6db, non bloquante) : le 0,25 de lecture.py:_dist_hvl_atr est ECRIT EN DUR — juste pour ES/NQ (meme tick), faux en silence le jour ou un instrument a tick different entre (RTY 0,10). Fix = lire le tick par symbole depuis CORE/constants.get_tick_size (la source unique de tick-size-policy), _dist_hvl_atr recoit le tick depuis lire(sym). Une ligne, avec les 3 commits pre-gel.
