...
**Cause racine** : un brief redige SANS acces au depot prive a designe un fichier PRECIS ; `V1_ARCHIVE/` contient une approche morte, gardee comme archive. Le code VIVANT est `BOT/dtc_connector.py` : trois ordres Type 208 separes + OCO manuel — et il porte en plus le fix du 04/05 (paire OCO enregistree AVANT l'envoi des enfants, apres un TP rempli en 596 ms sur NQ dont l'annulation du jumeau echouait en silence), que V1 n'a pas.
**Lecon** : ne JAMAIS designer un fichier precis de `CORE/` ou des archives dans un brief sans confirmer qu'il est vivant. Un brief dit « le connecteur qui tourne aujourd'hui », pas un nom de fichier. Et une archive n'est pas une source : elle est gardee parce qu'on l'a abandonnee.
**Trigger prevention** : (1) avant tout « copier X », `grep` ce que le code de production IMPORTE reellement ; (2) `exec_sim.py` IMPORTE `BOT/dtc_connector.py`, il ne le copie pas — une copie diverge en silence ; (3) l'en-tete d'`exec_sim.py` porte les deux corrections en clair, pour que la prochaine lecture ne reparte pas sur V1. Fable corrige sa facon d'ecrire les briefs (deuxieme « copier V1 » repris cette semaine, apres `atr_veille`).
**Reviewed** : Jackson (la question), Claude Code (la mesure), Fable (l'arbitrage — « c'est ma copie qui est prise en defaut, a corriger sans discuter »)
...
**RESOLU le 08/09 15:22 UTC** par Jackson en deux temps : (1) chart 15 remis en service -> `vix_level` revient (15,37) mais les 8 `dist_vix_*` restent nulles ; preuve isolee dans `DATA/vix_levels/.../vix.jsonl` : prix OK, tous les niveaux MQ a null. (2) niveaux recharges depuis l'app MenthorQ -> les 8 distances reviennent, verifiees UNE A UNE contre le graphique : HVL 19,50 = 15,49+4,01 ; Call 20,00 = +4,51 ; Put 14,50 = -0,99 ; Put 0DTE 16,00 = +0,51 ; HVL 0DTE 13,50 = -1,99 ; GEX 15,00/15,50 = -0,49/+0,01. `dist_vix_call_0dte` reste null — il l'etait DEJA le 15/05 sain, MenthorQ ne publie pas ce niveau (ne pas le flagger). **Reste a verifier par Jackson** : le chart affiche `Levels Timestamp: 2026-09-04` — les niveaux charges datent du jour ou la panne a commence.
**Reviewed** : Jackson (detection + reparation) / self (mesure, chaine d'impact, verification des 8 niveaux)
...
**Consumers prod degrades** : Bot 2 BN V4 (`bn_v4_engine.py:89-90,102-103` features ML), Bot 2 sierra_compat, Bot 3 v3 continuation Tier 3 (PREV_VAL+PREV_VAH proximity 0.02%), Bot 3 v3 sierra_reader, Bot 3 level_definitions, Bot 3 Gold Tier 2 (PREV_VAH+PREV_VAL+PDH+PDL proximity 0.0005%). Setups Tier 2-3 generes sur bruit aleatoire intra-session, pas vrais niveaux.
**Cause racine** : Plan agent Q2 identifie BUG critique design propose : `pdh != cash_high`. Sierra `pdh` inclut overnight (CME Daily High J-1 24h), `cash_high` = RTH only. Empirique 15/06 ES : pdh broadcast = 7560 = max(bar_high) 24h, cash_high = 7498. Source correcte = `max(bar_high)` 24h cumul session.
**Lecon** : apres detection d'un bug racine pattern Sierra natif (INCIDENT #75 pvwap), AUDITER PROACTIVEMENT toutes les features de meme famille `prev_*` AVANT que les consumers prod soient impactes. Manque de la systematique : audit 18/06 cherchait NULL/ZERO (facile) pas VARIES intra-session (subtil).
**Trigger prevention** : checklist post-deploy de tout fix override : grep tous les aliases `prev_*` / `pdh` / `pdl` Sierra du JSONL enriched + audit variance intra-session sur N>=3 jours. Si distinct > 1 par session = candidat extension du module override Python.
**Fix applique** : Module SSoT `CORE/phase_d_prev_helpers_streaming.py` (315 LOC) etend pattern PVWAP a 5 helpers. Sources : cur_vah/val/vpoc EOD pour VA, max(bar_high)/min(bar_low) 24h pour pdh/pdl. 2 feature flags separes (MIA_PREV_VA_WIRE_ENABLED + MIA_PREV_DAY_HL_WIRE_ENABLED) car sources differentes. Convention DMP signed (level - close)/close*100 = POSITIF si level au-dessus. Cohabitation composite_poc (INCIDENT #58) OK (trackers independants). 14/14 tests pytest PASS dont test_pdh_source_is_bar_high_not_cash_high (Q2) + test_dist_pct_sign_convention_dmp (regression #75). Test E2E 6957 bars ES 3 sessions : 0 erreur, session 15/06 prev_vah=1 distinct + pdh=1 distinct ✓. Verif live post-deploy session 19/06 ES bar 18:17:09 : 5/5 conventions DMP coherentes (prev_vah/val/vpoc + pdh/pdl tous signes corrects).
**Reviews agents** : Plan agent (GO-AVEC-RESERVES Q1-Q5 traitees) + code-reviewer (GO-AVEC-RESERVES mineures, R2 counter applique) + market-analyst (GO paper Sim2/Sim3, RESERVES live = backtest preservation 5j obligatoire avant promotion live car Bot 3 Gold proximity 0.0005% peut chuter -40 a -70% frequence trades).
**Reviewed** : Plan agent + code-reviewer + market-analyst 19/06.

### 2026-06-18 (75) - [VALIDATION_MISS] - Phase D Dalton 9 features 100% NULL ES+NQ (5 consumers prod degrades silencieusement depuis le debut)

**Contexte** : Audit empirique 30528 bars live (ES 5j + NQ 4j, 10-15/06) revele 9 features `phase_d_dalton_levels.py` (Single Prints + Naked POC) **100% NULL**. Consumers prod actifs : `bias_calculator_v6.py:754` (PTS_SP=0.10 + PTS_NAKED_POC=0.10 jamais appliques), `setup_engine.py:507`, `bot3_snapshot_recorder.py:39`, `bot3_level_definitions.py:29` Tier 1 SINGLE_PRINT, `bot3_gold_level_definitions.py:62` Bot 3 Gold. Pattern identique INCIDENT #74 (gamma_block) : module Python existe mais jamais wire-up live.
**Ce qui a mal tourne** : `phase_d_dalton_levels.py` est BATCH (pandas DataFrame.apply, groupby session) alors que `sierra_pipeline.py` pipeline ACTIF live est STREAMING (bar par bar via *State mutables). Le module n'a jamais ete porte. Sub-decouverte : un sous-module `phase_d_pvwap_streaming.py` EXISTAIT deja code (6 features PVWAP) mais idem jamais wire-up. Sous-bug Sierra natif : helpers `pvwap/sd1u/sd1d` Sierra DMP varient INTRA-SESSION (15/06 ES bar 0=7395.23, bar 2384=7466.16, bar 3178=7612.84) alors que par definition pvwap = EOD VWAP J-1 doit etre CONSTANT.
**Cause racine** : pas de test integration verifiant la presence systematique des fields ML requis par downstream consumers (memes pattern #74). `phase_d_dalton_levels` produit 17 features dont 4 Excess deja DROPPED 28/04 v3 (saturation 94.8% NQ sur bars 1m). Scope reel 13 features (6 PVWAP + 4 Naked POC + 3 Single Prints + 3 zones).
**Lecon** : avant tout deploy d'un consumer qui lit `bar.get("dist_X_pct")`, valider empirique que `dist_X_pct` est present ET non-NaN sur >95% des bars d'une session COMPLETE. Si une feature est dans un module batch non wire-up live = 100% NaN/NULL silencieux garanti. Faire grep `dataset_builder.PHASE_D_GENERATED` vs `sierra_pipeline.py` ordre d'execution apres chaque migration de pipeline.
**Trigger prevention** : checklist post-migration pipeline `feature_catalog vs sierra_pipeline.py` cross-check obligatoire. Si une feature listee dans dataset_builder generated set n'apparait pas dans le pipeline streaming = INCIDENT.
**Fix applique (Phase 1/3)** : wirage `CORE/phase_d_pvwap_streaming.py` dans `sierra_pipeline.py` (6 features PVWAP : 3 helpers + 3 distances). Verifie empirique 6717 bars enrich_bar : 0 erreur, distances non-NaN apres warmup 1 session (47% bars test = session 15/06 avec prev session vue), pvwap CONSTANT par session (override silencieux bug Sierra intra-session). Code-reviewer GO AVEC RESERVES R1+R3 appliquees : R1 = passer 6 keys explicites a `_safe_update` au lieu de filter `set - set` (sinon wirage placebo apres bar 1), R3 = env var `MIA_PVWAP_WIRE_ENABLED` defaut "1" pour rollback rapide si Bot 2 BN V4 paper Sim2 sur-trade demain. Phases 2+3 (Naked POC + Single Prints streaming) backlog separe (~7h).
**Sous-incident a tracer separement** : bug Sierra natif `pvwap` intra-session (DMP_Transform.h emit `pvwap` qui change intra-session au lieu d'etre constant). NON corrige cette session, fix Python override masque le bug live mais le bug C++ reste a investiguer.
**BONUS FIX detecte au pre-deploy 18/06 (review #2 code-reviewer + grep consumers Q3)** : `phase_d_pvwap_streaming.py:241-258` v1 (24/05) avait la convention de signe INVERSEE (`(close - pvwap) / close`) alors que la convention DMP/Bot 3 v3 fallback/Bot 2 BN V4 est `(level - close) / close * 100` (positif quand level au-dessus). Bug latent depuis 24/05 (module jamais wire-up) corrige avant d'etre actif. Si deploy avant fix = inversion silencieuse signal directionnel sur 3 features pour 5 consumers prod = catastrophe paper. Empirique post-fix : bar 15/06 ES close=7497.5, pvwap=7582.74 (au-dessus), dist_pvwap_pct=+1.13 POSITIF correct.
**Reviewed** : code-reviewer review #1 (GO AVEC RESERVES R1+R3) + code-reviewer review #2 (GO AVEC RESERVE MINEURE Q1 doc DEROGATION_POLICY) + Claude empirique (test integration enrich_bar 6717 bars + verif convention signe bar 15/06 OK) + Claude grep convention DMP_F3_DistNormalisees.h:101 vs Python = bug detecte.
