...
LOG_CODES = {
    ...
    # Phase 5.0.A Proxy MenthorQ Sierra (11/06/2026) — 3 codes
    # Cf CORE/menthorq_v2_sierra_proxy.py + DOCS/INCIDENT_LOG.md 2026-06-11 MENTHORQ_SCRAPER_DOWN
    "MQ_PROXY_INJECTED":           (LogLevel.INFO,     "decisions", "Proxy MenthorQ Sierra injecte gamma_condition={gamma} depuis flip_zone={flip} ({sym})"),
    "MQ_PROXY_SKIPPED_NO_FLIP_ZONE":(LogLevel.ALERTE,  "decisions", "Proxy MQ skip : bool_gex_flip_zone absent ({sym}). mq_gamma_condition reste None -> fail-closed downstream"),
    "MQ_PROXY_PRESERVED_ORIGINAL": (LogLevel.INFO,     "decisions", "Proxy MQ skip : mq_gamma_condition deja set (source originale Databento) ({sym})"),

    # Phase 5.0.B Proxy aggressor_imbalance Sierra (11/06/2026) — 3 codes
    # Cf CORE/aggressor_imbalance_proxy.py
    "AGGRESSOR_PROXY_INJECTED":            (LogLevel.INFO,    "decisions", "Proxy aggressor_imbalance Sierra injecte aggro={aggro:.4f} ({sym})"),
    "AGGRESSOR_PROXY_SKIPPED_MISSING_INPUTS":(LogLevel.ALERTE,"decisions", "Proxy aggressor skip : inputs manquants (buy_vol/sell_vol/avg_*_size). aggressor_imbalance reste None ({sym})"),
    "AGGRESSOR_PROXY_PRESERVED_ORIGINAL":  (LogLevel.INFO,    "decisions", "Proxy aggressor skip : aggressor_imbalance deja set (source originale Databento PhaseB+) ({sym})"),

    # Phase 0 Sierra Pipeline Port (11/06/2026) — 52 codes
    # Cf DOCS/superpowers/specs/2026-06-11-sierra-pipeline-port-phase0-design-v2.md
    # Per-module OK/FAIL/DEGRADED pour 13 modules (39 codes)
    "SIERRA_PORT_FOOTPRINT_BUILDER_OK":      (LogLevel.INFO,   "events", "Sierra port footprint_builder OK : sym={sym} n_cells={n_cells}"),
    "SIERRA_PORT_FOOTPRINT_BUILDER_FAIL":    (LogLevel.MAJEUR, "events", "Sierra port footprint_builder FAIL : sym={sym} err={err}"),
    "SIERRA_PORT_FOOTPRINT_BUILDER_DEGRADED":(LogLevel.ALERTE, "events", "Sierra port footprint_builder DEGRADED : sym={sym} reason={reason}"),
    "SIERRA_PORT_LOT1_TRADES_OK":            (LogLevel.INFO,   "events", "Sierra port LOT 1 trades OK : sym={sym} n_trades={n_trades}"),
    "SIERRA_PORT_LOT1_TRADES_FAIL":          (LogLevel.MAJEUR, "events", "Sierra port LOT 1 trades FAIL : sym={sym} err={err}"),
    "SIERRA_PORT_LOT1_TRADES_DEGRADED":      (LogLevel.ALERTE, "events", "Sierra port LOT 1 trades DEGRADED : sym={sym} reason={reason}"),
    "SIERRA_PORT_LOT2_BIG_V2_OK":            (LogLevel.INFO,   "events", "Sierra port LOT 2 big_v2 OK : sym={sym}"),
    "SIERRA_PORT_LOT2_BIG_V2_FAIL":          (LogLevel.MAJEUR, "events", "Sierra port LOT 2 big_v2 FAIL : sym={sym} err={err}"),
    "SIERRA_PORT_LOT2_BIG_V2_DEGRADED":      (LogLevel.ALERTE, "events", "Sierra port LOT 2 big_v2 DEGRADED : sym={sym} reason={reason}"),
    "SIERRA_PORT_LOT3_CLUSTER_V2_OK":        (LogLevel.INFO,   "events", "Sierra port LOT 3 cluster_v2 OK : sym={sym}"),
    "SIERRA_PORT_LOT3_CLUSTER_V2_FAIL":      (LogLevel.MAJEUR, "events", "Sierra port LOT 3 cluster_v2 FAIL : sym={sym} err={err}"),
    "SIERRA_PORT_LOT3_CLUSTER_V2_DEGRADED":  (LogLevel.ALERTE, "events", "Sierra port LOT 3 cluster_v2 DEGRADED : sym={sym} reason={reason}"),
    "SIERRA_PORT_LOT4_ABSORB_OK":            (LogLevel.INFO,   "events", "Sierra port LOT 4 absorb OK : sym={sym}"),
    "SIERRA_PORT_LOT4_ABSORB_FAIL":          (LogLevel.MAJEUR, "events", "Sierra port LOT 4 absorb FAIL : sym={sym} err={err}"),
    "SIERRA_PORT_LOT4_ABSORB_DEGRADED":      (LogLevel.ALERTE, "events", "Sierra port LOT 4 absorb DEGRADED : sym={sym} reason={reason}"),
    "SIERRA_PORT_LOT5_TRAPPED_OK":           (LogLevel.INFO,   "events", "Sierra port LOT 5 trapped OK : sym={sym}"),
    "SIERRA_PORT_LOT5_TRAPPED_FAIL":         (LogLevel.MAJEUR, "events", "Sierra port LOT 5 trapped FAIL : sym={sym} err={err}"),
    "SIERRA_PORT_LOT5_TRAPPED_DEGRADED":     (LogLevel.ALERTE, "events", "Sierra port LOT 5 trapped DEGRADED : sym={sym} reason={reason}"),
    "SIERRA_PORT_LOT6_DELTA_DIV_EXT_OK":     (LogLevel.INFO,   "events", "Sierra port LOT 6 delta_div_ext OK : sym={sym}"),
    "SIERRA_PORT_LOT6_DELTA_DIV_EXT_FAIL":   (LogLevel.MAJEUR, "events", "Sierra port LOT 6 delta_div_ext FAIL : sym={sym} err={err}"),
    "SIERRA_PORT_LOT6_DELTA_DIV_EXT_DEGRADED":(LogLevel.ALERTE,"events", "Sierra port LOT 6 delta_div_ext DEGRADED : sym={sym} reason={reason}"),
    "SIERRA_PORT_PHASE_B_HELPERS_OK":        (LogLevel.INFO,   "events", "Sierra port phase_b_helpers OK : sym={sym}"),
    "SIERRA_PORT_PHASE_B_HELPERS_FAIL":      (LogLevel.MAJEUR, "events", "Sierra port phase_b_helpers FAIL : sym={sym} err={err}"),
    "SIERRA_PORT_PHASE_B_HELPERS_DEGRADED":  (LogLevel.ALERTE, "events", "Sierra port phase_b_helpers DEGRADED : sym={sym} reason={reason}"),
    "SIERRA_PORT_PHASE_B_PLUS_LONG_OK":      (LogLevel.INFO,   "events", "Sierra port phase_b_plus_long OK : sym={sym}"),
    "SIERRA_PORT_PHASE_B_PLUS_LONG_FAIL":    (LogLevel.MAJEUR, "events", "Sierra port phase_b_plus_long FAIL : sym={sym} err={err}"),
    "SIERRA_PORT_PHASE_B_PLUS_LONG_DEGRADED":(LogLevel.ALERTE, "events", "Sierra port phase_b_plus_long DEGRADED : sym={sym} reason={reason}"),
    ...
    "SIERRA_PORT_EDGE_ZONES_FAIL":           (LogLevel.MAJEUR, "events", "Sierra port edge_zones FAIL : sym={sym} err={err}"),
    "SIERRA_PORT_EDGE_ZONES_DEGRADED":       (LogLevel.ALERTE, "events", "Sierra port edge_zones DEGRADED : sym={sym} reason={reason}"),
    "SIERRA_PORT_COLOR_BARS_OK":             (LogLevel.INFO,   "events", "Sierra port color_bars OK : sym={sym}"),
    "SIERRA_PORT_COLOR_BARS_FAIL":           (LogLevel.MAJEUR, "events", "Sierra port color_bars FAIL : sym={sym} err={err}"),
    "SIERRA_PORT_COLOR_BARS_DEGRADED":       (LogLevel.ALERTE, "events", "Sierra port color_bars DEGRADED : sym={sym} reason={reason}"),
    ...
    "SIERRA_STATE_SNAPSHOT_FAIL":            (LogLevel.CRITIQUE,"events", "Sierra state snapshot FAIL : sym={sym} err={err}"),
    "SIERRA_STATE_LOAD_FAIL":                (LogLevel.MAJEUR, "events", "Sierra state load FAIL : sym={sym} err={err} -> cold start"),
    "SIERRA_STATE_SCHEMA_MISMATCH":          (LogLevel.MAJEUR, "events", "Sierra state schema mismatch : sym={sym} loaded={loaded} expected={expected}"),
    "SIERRA_STATE_COLLISION_DETECTED":       (LogLevel.CRITIQUE,"events", "Sierra state collision Databento detectee : sym={sym} path={path} err={err}"),
    "SIERRA_FORMAT_ADAPTER_OK":              (LogLevel.INFO,   "events", "Sierra format adapter OK : sym={sym} fields_mapped={n}"),
    "SIERRA_FORMAT_ADAPTER_DROPPED":         (LogLevel.ALERTE, "events", "Sierra format adapter feature droppee : sym={sym} field={field} reason={reason}"),
    "SIERRA_FORMAT_TS_INCONSISTENT":         (LogLevel.MAJEUR, "data", "Sierra ts inconsistent : sym={sym} ts={ts} expected ms in [1e12,1e14]"),
    "SIERRA_VAP_TO_CELLS_OK":                (LogLevel.INFO,   "events", "Sierra VAP->cells OK : sym={sym} n_buckets={n_buckets} total_vol={total_vol}"),
    "SIERRA_VAP_TO_CELLS_EMPTY":             (LogLevel.ALERTE, "events", "Sierra VAP->cells empty : sym={sym} (VAP absent ou taille 0)"),
    ...
    # FIX 17/06 (Jackson audit logs) : GATE blocking codes -> "risk" pour fichier risk/risk_*_bot1v2.jsonl
    # dedie (avant : melangees avec decisions/, peu visibles dans audit risk).
    "BOT1V2_SKIP_HAS_POSITION":     (LogLevel.INFO,     "risk",      "Bot1V2 skip {sym} : position deja ouverte"),
    ...
    # FIX 17/06 (Jackson audit logs) : GATE blocking codes -> "risk" pour fichier risk/risk_*_bot_mr.jsonl dedie.
    "BOTMR_SKIP_HAS_POSITION":     (LogLevel.INFO,     "risk",      "BotMR skip {sym} : position deja ouverte"),
    "BOTMR_GATE_SESSION_BLOCK":    (LogLevel.INFO,     "risk",      "BotMR skip {sym} : session {phase} non-tradee"),
    ...
    # ULTRATHINK QUALITY FILTER (19/06/2026 - directive Jackson selectivite)
    "BOTMR_ULTRATHINK_BLOCK_DELTA_BAR_LONG": (LogLevel.INFO, "decisions",
        "BotMR {sym} LONG skip - ULTRATHINK delta_bar pas assez BEAR : delta={delta_bar} >= {threshold} (NEED MEAN_REVERT bar)"),
    "BOTMR_ULTRATHINK_BLOCK_DELTA_BAR_SHORT": (LogLevel.INFO, "decisions",
        "BotMR {sym} SHORT skip - ULTRATHINK delta_bar pas assez BULL : delta={delta_bar} <= {threshold} (NEED MEAN_REVERT bar)"),
    "BOTMR_ULTRATHINK_BLOCK_FINISH_STRENGTH_LONG": (LogLevel.INFO, "decisions",
        "BotMR {sym} LONG skip - ULTRATHINK no buyer recovery : finish={finish_strength} <= {threshold} (NEED RECOVERY end-of-bar)"),
    "BOTMR_ULTRATHINK_BLOCK_FINISH_STRENGTH_SHORT": (LogLevel.INFO, "decisions",
