...
LEVEL_MAP = {
    ...
    # 🔧 17/03/2026 Sprint 1 Fix #1: Score 2→1 (omniprésent 98% barres = zéro sélectivité)
    # Exige confluence avec un niveau Score 2+ pour fire
    'dist_ext_edge_buy':  ('EXT_EDGE_BUY',  1, 'support'),
    'dist_ext_edge_sell': ('EXT_EDGE_SELL',  1, 'resist'),
...
class EntryEngine:
    ...
    def compute(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 🔧 17/03/2026 Sprint 1 Fix #2: COOLDOWN STATE
        # Le bot C++ attend 10 min après un signal (cooldown_win/loss).
        # Sans cooldown, le Python fire 95% des signaux à <3 min d'écart.
        COOLDOWN_BARS = 10  # 10 barres = 10 min entre deux signaux
        last_signal_bar = -COOLDOWN_BARS  # Permet le premier signal immédiatement
        ...
        for i in range(len(df)):
            ...
            # 🔧 COOLDOWN: supprimer le signal si trop proche du précédent
            if sig.has_signal and (i - last_signal_bar) < COOLDOWN_BARS:
                sig = EntrySignal(bias_score=sig.bias_score,
                                  reason=f"COOLDOWN ({i - last_signal_bar}/{COOLDOWN_BARS} barres)")
            ...
            # Mettre à jour le cooldown si signal émis
            if sig.has_signal:
                last_signal_bar = i
    ...
    def _compute_bias(self, df: pd.DataFrame) -> pd.Series:
        ...
        🔧 17/03/2026 Sprint 1 Fix #3: Z-score ROLLING 30 barres
        AVANT: z-score sur tout le DF → Asia (sell-off) ancrait le biais London
        APRÈS: z-score glissant → le biais reflète les 30 dernières minutes
        """
        BIAS_LOOKBACK = 30  # 30 barres = 30 min de contexte
        ...
        for col, (base_w, domain) in CORE_FEATURES.items():
            ...
            vals = df[col]
            # 🔧 Rolling z-score au lieu de global
            roll_mean = vals.rolling(BIAS_LOOKBACK, min_periods=5).mean()
            roll_std = vals.rolling(BIAS_LOOKBACK, min_periods=5).std()
    ...
    def _evaluate_bar(self, row: pd.Series, bias: float, exhaust: dict = None) -> EntrySignal:
        ...
        # ═══ 🔧 17/03/2026 Sprint 1 Fix #4: VWAP VETO ═══
        # Bloque les trades contre la direction du VWAP (parité C++ L3 MIA_Layers.h)
        # Le VWAP slope positif = prix monte → SHORT interdit
        # Le VWAP slope négatif = prix baisse → LONG interdit
        # Seuil 0.5 tick/barre calibré sur les données 17/03 London
        # Note: ne bloque PAS les triggers autonomes (RVOL, Range, Exhaustion) ci-dessus
        VWAP_VETO_THRESHOLD = 0.5
        vwap_slope = row.get('ctx_vwap_slope_5', 0.0)
        if pd.isna(vwap_slope):
            vwap_slope = 0.0
        ...
        # 🔧 VWAP VETO: bloquer si direction contra-VWAP (parité C++ L3)
        if bias_dir == -1 and vwap_slope > VWAP_VETO_THRESHOLD:
            return EntrySignal(bias_score=bias,
                               reason=f"VWAP VETO: SHORT interdit (slope={vwap_slope:+.2f})")
        if bias_dir == 1 and vwap_slope < -VWAP_VETO_THRESHOLD:
            return EntrySignal(bias_score=bias,
                               reason=f"VWAP VETO: LONG interdit (slope={vwap_slope:+.2f})")
    ...
    def _calc_confidence(self, level: NearbyLevel, n_levels: int,
        ...
        if row is not None:
            ...
            # Failed Auction = le prix a tenté de sortir de la VA et a échoué
            # Signal de reversal ultra-fiable (Steidlmayer)
            if row.get('ctx_failed_auction', 0) == 1:
                conf += 0.08

            # POC Migration confirme la direction
            # POC monte + signal LONG = bonus | POC descend + signal SHORT = bonus
            poc_mig = row.get('ctx_poc_migration_10', 0) or 0
            if direction == 1 and poc_mig > 0.01:
                conf += 0.03
            if direction == -1 and poc_mig < -0.01:
                conf += 0.03

            # Rotation Factor élevé = jour de range → bonus pour mean reversion
            rot = row.get('ctx_rotation_factor_20', 0) or 0
            if rot >= 4:  # Beaucoup de rotations = range confirmé
                # Bonus si on trade vers le POC (mean reversion)
                vpoc_dist = row.get('dist_cur_vpoc', 0) or 0
                if (direction == 1 and vpoc_dist > 0) or (direction == -1 and vpoc_dist < 0):
                    conf += 0.05

            # IB Extension > 2.0 = trend day → pénalité pour mean reversion
            # Si on est loin de l'IB et on trade contra → prudence
            ib_ext = row.get('ctx_ib_extension_ratio', 1.0) or 1.0
            if ib_ext > 2.0:
                # En trend day, les signaux contra sont dangereux
                # Mais les signaux dans la direction du trend sont renforcés
                ib_dir = 1 if row.get('ib_broken_up', 0) == 1 else (-1 if row.get('ib_broken_down', 0) == 1 else 0)
                if ib_dir != 0 and direction == ib_dir:
                    conf += 0.05  # Trade avec le trend = bonus
                elif ib_dir != 0 and direction == -ib_dir:
                    conf -= 0.05  # Trade contra le trend = malus
