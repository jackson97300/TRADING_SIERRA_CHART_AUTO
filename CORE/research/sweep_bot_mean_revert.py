...
def detect_mean_revert_signal(
    bar: dict,
    *,
    sd_threshold_pct: float,  # ex 0.0 = "just touched", 0.05 = under SD2 by 0.05%
    rvol_zscore_min: float,
    require_exhaustion: bool,
    require_delta_direction: bool,
    use_sd_level: str = "sd2",  # "sd2" ou "sd3"
) -> Optional[str]:
    """Detecte signal mean revert. Retourne 'LONG' / 'SHORT' / None.
    ...
    # LONG : prix sous SD2d extension extreme low
    # dist_vwap_d_sd2d_pct <= -sd_threshold = prix sous le niveau SD2d
    if d_low <= -sd_threshold_pct:
        # Confirm delta achat (acheteurs reviennent)
        if require_delta_direction and delta_bar <= 0:
            return None
        return "LONG"

    # SHORT : prix au-dessus SD2u extension extreme high
...
def main():
    ...
    print(f"=== Top 15 configs {sym} (n>=5, tri PF * sqrt(n)) ===")
