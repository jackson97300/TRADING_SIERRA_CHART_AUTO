"""
edge_discovery_bot2.py — Edge discovery pour Bot 2 (Databento v4 enriched parquets)

Usage Claude Code :
    python edge_discovery_bot2.py --data-dir DATA/datasets/v4_enriched --symbol NQ --output DOCS/EDGE_REPORT_BOT2_NQ.md
    python edge_discovery_bot2.py --data-dir DATA/datasets/v4_enriched --symbol ES --output DOCS/EDGE_REPORT_BOT2_ES.md

Différences vs Bot 1 (edge_discovery.py) :
    - Source : parquet v4 enriched (418-419 features) au lieu de JSONL DMP
    - Features en _pct (pourcentage) au lieu de ticks absolus
    - Colonnes ctx_* (contextuelles), bn_* (battle navale), trapped_*, naked_poc_*
    - Structure : DATA/datasets/v4_enriched/symbol={SYM}.c.0/year=YYYY/month=MM/data.parquet
"""
...
def load_v4_parquets(data_dir: str, symbol: str = "NQ") -> pd.DataFrame:
    """Charge tous les parquets v4 enriched pour un symbole.

    Structure attendue :
      {data_dir}/symbol={SYM}.c.0/year=YYYY/month=MM/data.parquet
      ou {data_dir}/symbol={SYM}.c.0/year=YYYY/month=MM/day=DD/*.parquet
    """
    sym_full = f"{symbol}.c.0"
    patterns = [
        f"{data_dir}/symbol={sym_full}/**/data*.parquet",
        f"{data_dir}/symbol={sym_full}/**/*.parquet",
        f"{data_dir}/*{symbol}*/**/*.parquet",
        f"{data_dir}/**/*{symbol}*.parquet",
    ]
    ...
    if not files:
        ...
        print(f"  Structure attendue : {data_dir}/symbol={sym_full}/year=YYYY/month=MM/data.parquet")
        return pd.DataFrame()
...
def main():
    parser = argparse.ArgumentParser(description="Edge Discovery Bot 2 — Databento V4")
    parser.add_argument("--data-dir", required=True, help="Répertoire parquets v4 enriched")
    parser.add_argument("--symbol", default="NQ", help="NQ ou ES")
    parser.add_argument("--output", default="DOCS/EDGE_REPORT_BOT2.md")
    parser.add_argument("--horizon", type=int, default=15)
    parser.add_argument("--threshold", type=int, default=100, help="Seuil gros move ticks")
    parser.add_argument("--rth-only", action="store_true", help="US cash session only")
    parser.add_argument("--tick-value", type=float, default=0.50, help="NQ=0.50, ES=1.25")
    args = parser.parse_args()

    print("=" * 70)
    print(f"EDGE DISCOVERY BOT 2 — {args.symbol} (Databento V4)")
    print("=" * 70)
