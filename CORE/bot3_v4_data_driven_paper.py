...
# 10/06 Chantier persistance cross-bot (Jackson focus toute journee).
# Pattern Bot 3 v3 Sprint Phase 1 09/06 generalize a Bot 3 v4.
try:
...
class Bot3V4DataDrivenPaper:
    ...
    def __init__(
        ...
        # 03/06 Jackson : enable_swing depuis Bot3V4Params (default False)
        triggers = build_default_triggers(enable_swing=self.params.enable_swing_triggers)
        ...
        self._positions_persist = PositionPersistance(
            bot_name="bot3_v4", lock=self._pos_lock, emit_fn=_emit,
        )
        ...
        try:
            ...
            if restored:
                ...
                _emit("BOT3_V4_POSITIONS_RESTORED",
                      n_positions=len(restored),
                      symbols=",".join(restored.keys()))
                _emit("BOT3_V4_CID_INDEX_REBUILT", n_cids=len(self._cid_index))
            ...
            for sym in self.symbols:
                ...
                try:
                    ...
                    _emit("BOT3_V4_COOLDOWN_RESTORED",
                          symbol=sym,
    ...
    def boot_ready(self) -> None:
        ...
        10/06 Chantier persistance cross-bot — pattern Bot 3 v3.

        Sequence :
          1. Check flag file STATE/bot3_v4/force_flat.flag (consume-and-delete)
          ...
          5. Si HALT : emit BOT3_V4_HALT_BOOT_REQUIRES_HUMAN (CRITIQUE)
        """
        force_flat = False
        force_flat_flag_path = ROOT / "STATE" / "bot3_v4" / "force_flat.flag"
        if force_flat_flag_path.exists():
            force_flat = True
            try:
                force_flat_flag_path.unlink()
                _emit("BOT3_V4_FORCE_FLAT_FLAG_CONSUMED")
            except OSError as e:
                _emit("BOT3_V4_LOOP_ERROR",
                      err=f"force_flat_flag unlink: {str(e)[:200]}")

        env_force_flat = os.environ.get(
            "MIA_BOT3_V4_RECONCILE_FORCE_FLAT", "0") == "1"
            ...
            try:
                ...
                for r in reports:
                    if r.is_critical:
                        ...
                        _emit("BOT3_V4_HALT_BOOT_REQUIRES_HUMAN",
    ...
    def poll_cycle(self) -> None:
        ...
        if self._kill_switch_active:
            return
        # 10/06 Chantier persistance : halt_reason check (skip cycle si HALT)
        ...
        if not self._reconciled:
            ...
            if (self._last_poll_skip_reconcile_emit_ts is None or
                ...
                _emit("BOT3_V4_POLL_SKIP_RECONCILE")
    ...
    def _place_bracket_dtc(
        ...
        # 02/06 FIX C3 sizing per-bot : qty dynamic depuis GUARD_RAILS_BOT3
        try:
        ...
        with self._pos_lock:
            ...
            self._position[sym] = {
                ...
                "qty": qty,              # 02/06 FIX C3 dynamic
                "n_contracts": qty,       # 02/06 FIX C1 pour compute_pnl_R_usd
                "signal_id": signal_id,
        ...
        # 10/06 Chantier persistance : save position pour cross-restart safety
        try:
            self._positions_persist.save_position(sym, self._position[sym])
        except Exception as e:
            _emit("BOT3_V4_LOOP_ERROR",
                  sym=sym, err=f"save_position: {type(e).__name__}: {str(e)[:200]}")
    ...
    def handle_dtc_fill(self, msg: dict, cid: str) -> bool:
        ...
        with self._pos_lock:
            ...
            # 09/06 BUG #5 fix propagation : tick_value depuis GUARD_RAILS_BOT3
            # (Bot 3 v4 trade Cross Chart Micro NQ $0.50/tick comme Bot 3 v3)
            try:
        ...
        # 10/06 Chantier persistance : remove + save cooldown meta
        try:
            ...
            self._positions_persist.set_meta(
                f"last_trade_close_pnl_R_{sym}", round(pnl_R, 3))
        except Exception as e:
            _emit("BOT3_V4_LOOP_ERROR",
    ...
    def _force_close_position(self, sym: str, pos: dict, reason: str) -> None:
        ...
        # jamais declenche -> bot reouvre immediatement. Treat as loss (pnl_R=0)
        # pour appliquer cooldown LOSS=15min (conservateur).
        self._last_close_ts[sym] = datetime.now(timezone.utc)
        ...
        if self.dry_run or self.dtc is None:
            ...
            with self._pos_lock:
                ...
                self._position[sym] = None
                self._current_signal_id[sym] = None
            # 10/06 Chantier persistance : remove position fantome + save cooldown
            try:
                self._positions_persist.remove_position(sym)
                self._positions_persist.set_meta(
                    f"last_trade_close_ts_{sym}",
                ...
                _emit("BOT3_V4_LOOP_ERROR",
                      sym=sym, err=f"remove_position(dry_run timeout): {type(e).__name__}: {str(e)[:200]}")
            return
