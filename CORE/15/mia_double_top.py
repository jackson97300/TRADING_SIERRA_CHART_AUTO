...
class RetestConfig:
    ...
    min_bars_between_swings: int = 5          # Au moins 5 min entre les 2 sommets
...
def detect_double_top_bottom(
    ...
    for b in window:
        ...
        # Reconstruire high/low depuis les distances swing si pas de OHLC direct
        # Pour l'instant on utilise price comme proxy (sera amélioré avec OHLC)
        sh_dist = b.get("dist_swing_high", 0.0)
        sl_dist = b.get("dist_swing_low", 0.0)
        
        # dist_swing_high = (swing_high - price) en ticks → swing_high = price + dist * tick_size
        # Mais on a besoin du high de CETTE barre, pas du swing global
        # Approximation : on utilise le prix ± une fraction de l'ATR
        atr = b.get("atr", 10.0)
        bar_range_approx = atr * tick_size * 0.05  # ~5% ATR par barre 1-min
...
def run_tests():
    ...
    assert_near("T14 - weak boost", score, 0.08)
