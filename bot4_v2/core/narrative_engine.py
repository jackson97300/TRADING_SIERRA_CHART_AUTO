"""Bot 4 v2 narrative_engine — FORK CORE/narrative_engine.py (12/06) + adaptations.

FORK Phase P2 Batch 3b (25/06/2026) :
- Isolation totale bot4_v2 (pas d'import CORE/* sauf TICK_SIZE source unique GELE spec section 5)
- Logger via bot4_v2.observability.telemetry (PAS stdlib logging direct)
- Module expose bot4_v2.core.narrative_engine

Heritage V1 conserve (Phase B audit ULTRATHINK 12/06) :
- NarrativeContext dataclass = OBSERVATIONS STRUCTUREES reproductibles
- PAS de hardcoded gates / decisions trade (anti-PATTERN_11)
- 6 dataclasses : KeyLevel, MarketStructureState, OrderFlowState, SessionState,
  PatternsDetected, NarrativeContext
- Clustering confluence 10% ATR (anti-merge agressif)
- raise ValueError si bar invalide (anti VALIDATION_MISS silencieux)
- Sample log warnings 1/1000 anti-spam (random.random() < _WARN_SAMPLE_RATE)
- Reconstruction prix absolus depuis dist_* (swing_low, swing_high)
- ATR intraday convertit ticks -> points (fix unite Plan C 27/05)
- bn_strength compteurs (vs scores arbitraires)

# Architecture

NarrativeContext (dataclass top-level) contient :
  - timestamp + symbol + close + atr
  - key_levels_resistance / key_levels_support : List[KeyLevel] triees + clusterisees
  - market_structure : MarketStructureState (open relation, profile, day type, IB)
  - order_flow : OrderFlowState (delta day vs session, bias macro, BN strength)
  - session : SessionState (London, Judas, segment)
  - patterns : PatternsDetected (single prints, FVG, sweeps)
  - macro_regime : str (VIX-based)
  - vwap bands raw + swing levels (pour scenarios v2)

# Usage

    bar = ... # bar live_enriched JSONL (575+ features post Phase A)
    context = build_narrative_context(bar)
    # context.key_levels_resistance[0] = niveau resistance le plus proche
    # context.market_structure.profile_shape = "P"
    # context.order_flow.bn_bull_strength = 2
"""
...
# TICK_SIZE est source unique GELE (spec section 5 garde-fou). Import direct OK.
try:
    from CORE.constants import get_tick_size as _get_tick_size
except ImportError:
    from constants import get_tick_size as _get_tick_size  # type: ignore
...
class KeyLevel:
    ...
    confluence_count: int = 1  # niveaux clusterises (<=10% ATR)


@dataclass
class MarketStructureState:
    """Etat structure marche (Dalton + Steidlmayer)."""
    open_relation: str = "UNKNOWN"
    ...
    # Open Type Dalton (Tier 1 INVENTAIRE)
    # R2 review batch 3b P2 : annotation Optional[int] honnete vs source C++
    # DMP_OpenType.h (encode int 0=UNKNOWN, 1=OD_UP, 2=OD_DOWN, 3=OTD, etc.).
    # CORE/narrative_engine.py:80 declarait Optional[str] mais stockait int -
    # bug heritage CORE corrige dans le FORK.
    open_type: Optional[int] = None
    open_drive: bool = False
    open_bias_conf: float = 0.0
    open_direction: int = 0
    # Initial Balance Dalton (21 features inutilisees pre-fork)
...
class OrderFlowState:
    ...
    delta_div_event: int = 0
    finish_strength: float = 0.0
    # Battle Navale extended (souverain Jackson section 4-5 manuel)
    bn_color_up: int = 0
    bn_color_dn: int = 0
    bn_long_up: int = 0
    bn_long_dn: int = 0
    ...
    bn_bull_strength: int = 0  # compteur derive (Lot A1, anti-PATTERN_11)
...
class NarrativeContext:
    ...
    atr: float
    atr_14m: float
    key_levels_resistance: list = field(default_factory=list)
    ...
    vix_level: float = 0.0
    # VWAP bands brutes (scenarios SD2/SD3 touch reversal)
    ...
    # Niveaux d'invalidation structurels (stop_level par scenario)
...
# Tolerance clustering niveaux = 10% ATR (anti-merge agressif).
# Review market-analyst + code-reviewer 12/06 : 0.5 ATR avalait 6 sources sur
# 89 pts NQ (47% ATR). 0.10 = ~18 pts NQ live = cluster honnete.
CONFLUENCE_ATR_FRAC = 0.10
...
def _maybe_warn(key: str, default, exc: Optional[Exception] = None) -> None:
    """Echantillonne 1/1000 les fallbacks pour audit (anti VALIDATION_MISS).
...
def _pvwap(bar: dict) -> Optional[float]:
    """Helper PVWAP avec fallback canonique Sierra DMP.

    prev_vwap = nom historique. pvwap = alias Sierra DMP (schema 3.7.22+).
    `or` filtre aussi prev_vwap=0.0 invalide.
    """
    ...
    for i, level in enumerate(levels):
        ...
        for j in range(i + 1, len(levels)):
            ...
            if abs(other.price - level.price) / atr <= CONFLUENCE_ATR_FRAC:
                ...
                used[j] = True
        used[i] = True
        # Cluster level = mediane prices + sources fusionnees
...
def _extract_market_structure(bar: dict) -> MarketStructureState:
    ...
    open_drive : derive depuis open_type (1=OD_UP, 2=OD_DOWN). Le JSONL n'emet
    pas de champ separe `open_drive` (verifie schema 3.7.22+), mais
    DMP_OpenType.h ligne 261-263 encode OD direct dans open_type.
    """
    open_type_int = _safe_int(bar, "open_type")
    is_open_drive = open_type_int in (1, 2)
    # range_pos_va : normalisation %  (si > 1.5 = echelle %) ou ratio [0,1].
    range_pos_raw = _safe_float(bar, "range_pos_va")
    range_pos_normalised = (
        range_pos_raw / 100.0 if range_pos_raw > 1.5 else range_pos_raw
    )
    return MarketStructureState(
        open_relation=OPEN_RELATION_MAP.get(
            _safe_int(bar, "open_relation_type"), "UNKNOWN"
        ),
        profile_shape=PROFILE_SHAPE_MAP.get(
            _safe_int(bar, "profile_shape"), "UNKNOWN"
        ),
        ...
        range_pos_va_pct=range_pos_normalised,
        ...
        ib_broken_down=_safe_bool(bar, "ib_broken_down")
        or _safe_bool(bar, "ib_broken_dn"),
...
def _extract_order_flow(bar: dict) -> OrderFlowState:
    ...
    # Strength compteurs (anti-PATTERN_11 : count, pas score arbitraire)
    bn_bull_strength = (
...
def build_narrative_context(bar: dict) -> NarrativeContext:
    ...
    if not bar or not isinstance(bar, dict):
        raise ValueError(
            "build_narrative_context: bar empty or invalid "
            f"(type={type(bar).__name__})"
        )
    close = _safe_float(bar, "close")
    # Sierra emet deux ATR, tous deux en TICKS :
    #   - "atr"     = ATR 14 DAILY  (ex NQ ~784 ticks = ~196 pts/jour)
    #   - "atr_14m" = ATR 14 intraday (ex NQ ~50 ticks = ~12 pts)
    # ctx.atr = ATR intraday EN POINTS (unite distance ctx), sinon stops 16x trop
    # larges (RR casse, confluence sur-mergee). Bug d'unite famille incident
    # Plan C 27/05.
    sym = bar.get("sym", "NQ")
