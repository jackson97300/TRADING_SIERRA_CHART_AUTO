...
inline SLTPResult CalculateProtectedSLTP(
    ...
    */
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // 🆕 31/01/2026: SESSION POC/VAH/VAL - Niveaux intraday de protection
    // Ces niveaux sont très pertinents pour placer SL/TP en intraday
    // ═══════════════════════════════════════════════════════════════════════════
    if (bn.session_poc > 0) levels.push_back({bn.session_poc, "SESSION_POC"});
    if (bn.session_vah > 0) levels.push_back({bn.session_vah, "SESSION_VAH"});
    if (bn.session_val > 0) levels.push_back({bn.session_val, "SESSION_VAL"});
    
    // 🆕 Previous Day Levels - Niveaux J-1 de protection
    if (mq.prev_vpoc > 0) levels.push_back({mq.prev_vpoc, "PREV_VPOC"});
    if (mq.prev_vah > 0) levels.push_back({mq.prev_vah, "PREV_VAH"});
    if (mq.prev_val > 0) levels.push_back({mq.prev_val, "PREV_VAL"});
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // 🆕 02/02/2026: SWING HIGH/LOW - NIVEAUX DE STRUCTURE DOW THEORY
    // Les Swing H/L représentent les pivots de structure du marché.
    // Si le prix casse un Swing, la structure est invalidée → excellent pour SL
    // ═══════════════════════════════════════════════════════════════════════════
    if (bn.swing_high > 0) {
        levels.push_back({bn.swing_high, "SWING_HIGH"});  // Résistance structurelle
    }
    if (bn.swing_low > 0) {
        levels.push_back({bn.swing_low, "SWING_LOW"});    // Support structurel
    }
    
    // ═══════════════════════════════════════════════════════════════════════════
    // 🆕 02/02/2026: SESSION HIGH/LOW - NIVEAUX PSYCHOLOGIQUES INTRADAY
    // Tous les traders surveillent ces niveaux = réactions probables
    // ═══════════════════════════════════════════════════════════════════════════
    if (bn.session_high > 0) {
        levels.push_back({bn.session_high, "SESSION_HIGH"});  // Résistance session
    }
    if (bn.session_low > 0) {
        levels.push_back({bn.session_low, "SESSION_LOW"});    // Support session
    }
