    ...
    SCStudyInterfaceRef sc, float price_close, bool ib_complete)
{
    ...
    if (cached != DMP_OT_NOT_SET) return cached;
...
inline void DMP_UpdateOpenType(
    ...
    DMP_MLFeatures&     f)
{
    // Extraire l'heure depuis la BARRE (timestamp données marché) — PAS l'horloge PC
    // sc.BaseDateTimeIn[sc.Index] = datetime de la barre courante (heure trading = ET)
    // Note : Si Sierra Chart est configuré en UTC, les comparaisons 9h30/10h30/16h00
    //        resteront correctes car DMP_Reader calcule déjà time_et de la même façon.
    SCDateTime bar_dt = sc.BaseDateTimeIn[sc.Index];
    int h = 0, m = 0, s_sec = 0;
    bar_dt.GetTimeHMS(h, m, s_sec);
    const int time_et = h * 60 + m;
    ...
    if (cached_oz == DMP_OT_NOT_SET && DMP_IsPriceValid(r.open_cash)) {
    ...
    f.open_type      = ot;
    f.open_zone      = (cached_oz != DMP_OT_NOT_SET) ? cached_oz : OZ_AT_POC;
