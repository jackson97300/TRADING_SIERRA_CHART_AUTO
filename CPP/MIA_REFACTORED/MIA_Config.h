...
inline const char* MIA_SCHEMA_VERSION = "3.6.0";
inline const char* MIA_BUILD_DATE = "2026-03-14";
inline const char* MIA_BUILD_REFACTORED = "v2.0-SESSION_PLANNER";
...
struct SymbolConfig {
    ...
    // Cooldown — 🆕 14/03/2026: Aligné sur sim Python (barres ≈ minutes)
    ...
    // Layers (ancien paradigme — sera remplacé par confluence)
    float l2_bn_score_min_long;
    float l2_bn_score_max_short;
    float l4_combo_required;

    // 🆕 14/03/2026: Session Planner — prouvé par sim PF 1.79
    int   max_trades_per_day;        // Global (NQ+ES combinés)
    int   max_trades_per_session;    // Per session (London, US_Active, etc.)
    float momentum_filter_pts;       // Anti-trend filter (pts, pas ticks)
    int   momentum_filter_bars;      // Lookback pour le filter
    int   min_confirmations;         // Minimum de confirmations confluence
};
...
inline const SymbolConfig CONFIG_ES = {
    ...
    // SL/TP — 🆕 14/03: Aligné sim (SL 30-50t, TP dynamic)
    20,         // sl_default_ticks (5 pts)
    12,         // sl_min_ticks (3 pts) — 🆕 abaissé de 16→12 (sim ES = SL moy 15t)
    28,         // sl_max_ticks (7 pts)
    3,          // sl_buffer_ticks
    24,         // tp_default_ticks (6 pts)
    32,         // tp_max_ticks (8 pts)
    2,          // tp_buffer_ticks
    0.80f,      // min_rr_ratio — 🆕 abaissé de 1.20→0.80 (aligné sim)

    // TRAILING — 🆕 14/03: Aligné sim ES (8/5)
    8,          // trailing_activation_ticks (était 15)
    5,          // trailing_distance_ticks (était 8)

    // BREAK-EVEN
    10,         // break_even_activation_ticks
    1,          // break_even_buffer_ticks

    // COOLDOWN — 🆕 14/03: Aligné sim (3/5 barres ≈ 3/5 min)
    3,          // max_consecutive_losses
    30,         // cooldown_after_losses_min (était 45)
    3,          // cooldown_win_min (était 10 — sim = 3 barres)
    5,          // cooldown_loss_min (était 15 — sim = 5 barres)

    // ANNONCES
    4,          // spread_alert_ticks
    30,         // dom_min_depth

    // LAYERS (ancien — sera remplacé par confluence)
    -0.10f,     // l2_bn_score_min_long (était -0.05 — sim montre -0.1 est le seuil)
    0.10f,      // l2_bn_score_max_short (était 0.05)
    1,          // l4_combo_required

    // 🆕 SESSION PLANNER
    16,         // max_trades_per_day (NQ+ES combinés)
    5,          // max_trades_per_session (per London, US_Active, etc.)
    1.5f,       // momentum_filter_pts (ES bouge ~4x moins que NQ)
    3,          // momentum_filter_bars
    2,          // min_confirmations (Edge+Color minimum)
};
...
inline const SymbolConfig CONFIG_NQ = {
    ...
    // SL/TP — 🆕 14/03: Aligné sim (SL moy 39t)
    28,         // sl_default_ticks (7 pts)
    20,         // sl_min_ticks (5 pts)
    50,         // sl_max_ticks (12.5 pts) — 🆕 augmenté de 40→50 (sim = jusqu'à 48t)
    5,          // sl_buffer_ticks
    35,         // tp_default_ticks (8.75 pts)
    50,         // tp_max_ticks (12.5 pts)
    3,          // tp_buffer_ticks
    0.80f,      // min_rr_ratio — 🆕 abaissé de 1.25→0.80 (aligné sim)

    // TRAILING — 🆕 14/03: Aligné sim NQ (20/12)
    20,         // trailing_activation_ticks (était 35 — sim = 20)
    12,         // trailing_distance_ticks (OK)

    // BREAK-EVEN
    15,         // break_even_activation_ticks
    2,          // break_even_buffer_ticks

    // COOLDOWN — 🆕 14/03: Aligné sim (3/5 barres)
    3,          // max_consecutive_losses
    30,         // cooldown_after_losses_min (était 45)
    3,          // cooldown_win_min (était 10 — sim = 3 barres)
    5,          // cooldown_loss_min (était 15 — sim = 5 barres)
    ...
    // LAYERS (ancien — sera remplacé par confluence)
    -0.10f,     // l2_bn_score_min_long (était -0.05 — sim montre -0.1)
    0.10f,      // l2_bn_score_max_short (était 0.05)
    1,          // l4_combo_required

    // 🆕 SESSION PLANNER
    16,         // max_trades_per_day
    5,          // max_trades_per_session
    5.0f,       // momentum_filter_pts (NQ bouge ~4x plus que ES)
    3,          // momentum_filter_bars
    2,          // min_confirmations
};
