...
// - Hiver (EST): UTC - 5h → FR_TO_ET_OFFSET = 5  (nov → mars)
// - Été (EDT): UTC - 4h → FR_TO_ET_OFFSET = 4    (mars → nov)
// Formule: now_min_et = now_min_utc - FR_TO_ET_OFFSET * 60
// 🆕 14/03/2026: EDT actif depuis le 8 mars 2026 (2nd dimanche de mars)
//    Prochain changement → 1er novembre 2026 (repasser à 5)
inline const int FR_TO_ET_OFFSET = 4;  // 🔧 EDT (été) depuis 08/03/2026
...
// 🆕 14/03/2026: Aligné avec MIA_SessionPlan.h (11 phases)
// Retourne un nom court pour les logs et le dashboard
inline const char* GetCurrentSessionName(SCStudyInterfaceRef sc) {
    int now_min = GetMinutesSinceMidnightET(sc.CurrentSystemDateTime);

    // 19h00-20h00 ET = Asia Warmup
    if (now_min >= 19 * 60 && now_min < 20 * 60) return "Asia_Warmup";
    // 20h00-22h00 ET = Asia IB (range institutionnel)
    if (now_min >= 20 * 60 || now_min < 22 * 60 - 24 * 60) {
        // Wrap midnight: 20h-22h
        if (now_min >= 20 * 60) return "Asia_IB";
    }
    // Simplified: use time ranges
    if (now_min >= 20 * 60 || now_min < 2 * 60) {
        if (now_min >= 20 * 60 && now_min < 22 * 60) return "Asia_IB";
        return "Asia";
    }
    if (now_min >= 2 * 60 && now_min < 2 * 60 + 30) return "Pre_London";
    if (now_min >= 2 * 60 + 30 && now_min < 3 * 60 + 15) return "London_Trans";
    if (now_min >= 3 * 60 + 15 && now_min < 4 * 60 + 30) return "London";
    if (now_min >= 4 * 60 + 30 && now_min < 9 * 60 + 30) return "London";
    if (now_min >= 9 * 60 + 30 && now_min < 9 * 60 + 45) return "US_Trans";
    if (now_min >= 9 * 60 + 45 && now_min < 10 * 60 + 30) return "US_IB";
    if (now_min >= 10 * 60 + 30 && now_min < 12 * 60) return "US_Active";
    if (now_min >= 12 * 60 && now_min < 14 * 60) return "Mid_AM";
    if (now_min >= 14 * 60 && now_min < 16 * 60) return "US_PM";
    return "Closed";
}
