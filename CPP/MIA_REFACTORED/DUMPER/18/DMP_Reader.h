...
struct DMP_RawData {
    ...
    float vix_call_0dte;                 // sg5 VIX Chart15 — Call 0DTE VIX
    float vix_put_0dte;                  // sg6 VIX Chart15 — Put 0DTE VIX
    float vix_hvl_0dte;                  // sg7 VIX Chart15 — HVL 0DTE VIX
    float vix_gex[10];                   // sg9..18 VIX Chart15 — GEX 1-10 VIX
...
inline void DMP_ReadSession(SCStudyInterfaceRef sc, DMP_RawData& d) {
    ...
    // ⚠️ FIX Bug #19 (27/03/2026) — SafeReadLast → SafeReadLastValid
    //    "High/Low for Time Period 9h30-10h30" se comporte comme OVN :
    //    après 10h30, arr[last_bar]=0 car la fenêtre est fermée.
    //    SafeReadLastValid scanne en arrière pour trouver la valeur gelée à 10h30.
    //    Identique au fix OVN déjà en production. Zéro régression.
    ...
    d.ib_high = DMP_SafeReadLastValid(sc, chart_b, hh_cash, 0);   // index 0 = High (gelé à 10h30) ✅ fix final
    d.ib_low  = DMP_SafeReadLastValid(sc, chart_b, ll_cash, 1);   // index 1 = Low  (gelé à 10h30) ✅ fix final
...
inline void DMP_ReadAll(SCStudyInterfaceRef sc, DMP_RawData& d, bool is_nq) {
    ...
    //  UTC-5 (EST) le reste de l'année. Fix 11/03/2026.
    {
        ...
        //  Utilise tm_wday pour détection EXACTE du DST US.
        //  Règle: 2ème dimanche mars → 1er dimanche novembre.
        //  prev_sunday = dy - wday → date du dimanche le plus récent.
        //    Avr-Oct: toujours DST.
        //    Mars: prev_sunday >= 8 → le 2ème dimanche est passé → DST actif.
        //    Nov: prev_sunday < 1 → le 1er dimanche n'est pas passé → DST actif.
        //  Testé exhaustivement sur 2024-2028 : zéro erreur.
        bool is_dst = false;
        {
            int mo = gm->tm_mon + 1;   // tm_mon = 0-11, on veut 1-12
            int dy = gm->tm_mday;
            int wday = gm->tm_wday;    // 0=Sun, 1=Mon, ..., 6=Sat
        ...
        // Calé sur l'heure FR : minuit FR = 18:00 ET
        //   Asia:   18:00-03:00 ET (00:00-09:00 FR)
        //   London: 03:00-09:00 ET (09:00-15:00 FR)
        //   US:     09:00-18:00 ET (15:00-00:00 FR)
        if (h_et >= 18 || h_et < 3)
            d.session = 0;  // Asia
