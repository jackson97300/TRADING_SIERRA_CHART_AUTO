...
//
//  Portée : Session courante (toutes sessions : Asia, London, RTH, OVN).
//           FIX 03/03/2026 : supprimé le guard RTH-only pour alimenter le ML overnight.
//
...
inline void DMP_ComputeHVN_LVN(
    ...
    // ⚠️ FIX 03/03/2026 : supprimé le guard is_rth_session
    //   AVANT : HVN/LVN calculé uniquement en RTH (09h30-16h00 ET)
    //   → 4 features null pendant ~15h/jour (Asia + London + Pre-market)
    //   MAINTENANT : calcul sur TOUTES les sessions.
    //   En Asia, le range est plus étroit mais VolumeAtPriceForBars a des
    //   données valides. Le ML bénéficie de features non-null même overnight.
    bool do_recalc = is_new_bar_closed
