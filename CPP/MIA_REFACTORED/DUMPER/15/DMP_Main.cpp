...
if (sc.SetDefaults) {
    ...
    sc.StudyDescription      = "Collecte 195 features ML en JSONL (G1-G13). "
...
// 🆕 BUG #13 FIX (05/03/2026) : Figer IB high/low à 10:30 ET
//   Avant : HH_CASH/LL_CASH trackent toute la cash session → IB monte après 10:30
//   Après : on capture IB une seule fois quand ib_complete passe à true
{
    constexpr int DMP_PS_IB_HIGH   = 117;
    constexpr int DMP_PS_IB_LOW    = 118;
    constexpr int DMP_PS_IB_FROZEN = 119;  // 1.0 = figé, 0.0 = pas encore

    float& frozen  = sc.GetPersistentFloat(DMP_PS_IB_FROZEN);
    float& ib_h    = sc.GetPersistentFloat(DMP_PS_IB_HIGH);
    float& ib_l    = sc.GetPersistentFloat(DMP_PS_IB_LOW);

    if (r.ib_complete && r.is_rth_session && frozen != 1.0f 
        && DMP_IsPriceValid(r.ib_high) && DMP_IsPriceValid(r.ib_low)) {
        // Première barre avec IB complete → capturer et figer
        ib_h = r.ib_high;
        ib_l = r.ib_low;
        frozen = 1.0f;
    }
    ...
    if (frozen == 1.0f) {
        // Écraser les valeurs lues avec les valeurs figées
        r.ib_high = ib_h;
        r.ib_low  = ib_l;
    }

    // Reset quand on est hors RTH (avant 9:30 ou après 16:00)
    // Ça couvre le passage de session overnight → nouvelle journée
    if (!r.is_rth_session && frozen == 1.0f) {
        frozen = 0.0f;
        ib_h = DMP_INVALID;
        ib_l = DMP_INVALID;
    }
...
// ═══════════════════════════════════════════════════════════════════════════════
//   ⚠️ TEMPORAIREMENT DÉSACTIVÉ — PersistentFloat slots 170+ crash possible
//   Les defaults sont écrits (rvol=1.0, signaux=0).
//   Le RVOL est calculé côté Python via rvol.py (mêmes formules).
//   TODO: Réactiver quand on aura confirmé la limite PersistentFloat SC.
#if 0
{
...
#endif
// RVOL defaults déjà mis dans DMP_Transform.h (rvol=1.0, signaux=0)
// Python rvol.py recalcule le vrai RVOL à partir de total_vol dans le JSONL
