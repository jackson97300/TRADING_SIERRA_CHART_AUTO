...
struct DMP_MLFeatures {
    ...
    // 🆕 FIX 07/03/2026 — Compteurs + cluster density gros ordres
    // Les prix bruts sont dans bn_ask100[10] etc. mais on ne gardait que la distance.
    // On ajoute les COMPTEURS (combien?) et CLUSTER DENSITY (combien proches du prix?).
    float n_big_ask;                  // Nombre total de gros ordres ASK (tous seuils)
    float n_big_bid;                  // Nombre total de gros ordres BID (tous seuils)
...
static inline void CalcBatailleNavale(const DMP_RawData& r, DMP_MLFeatures& f) {
    ...
    // NQ: +10, +30, +100  |  ES: +100, +150, +400, +1000
    {
        ...
        // 🆕 FIX 07/03/2026 — Compteurs + cluster density gros ordres
        // Ancien système avait ask_100=3, bid_30=5, etc. = COMPTEURS par seuil
        // + ask_100_prices[0..9] = PRIX individuels.
        // Le nouveau ne gardait que dist_big_*_nearest = on perdait le clustering.
        // Maintenant on ajoute: n_total + cluster dans rayon 20t et 50t.
        {
            const float* all_ask[] = { r.bn_ask100, r.bn_ask_extra1, r.bn_ask_extra2, r.bn_ask_extra3 };
            const float* all_bid[] = { r.bn_bid100, r.bn_bid_extra1, r.bn_bid_extra2, r.bn_bid_extra3 };

            float n_ask = 0, n_bid = 0;
            ...
            for (int t = 0; t < 4; t++) {
                for (int i = 0; i < 10; i++) {
                    // ASK
                    float ap = all_ask[t][i];
                    if (DMP_IsPriceValid(ap)) {
                        n_ask += 1.0f;
                        float d = (ap - p) / ts;  // distance en ticks (positif = au-dessus)
                        if (d > 0 && d <= 20) ask_c20 += 1.0f;
                        if (d > 0 && d <= 50) ask_c50 += 1.0f;
                    }
                    // BID
                    float bp = all_bid[t][i];
                    if (DMP_IsPriceValid(bp)) {
                        n_bid += 1.0f;
                        float d = (p - bp) / ts;  // distance en ticks (positif = en-dessous)
                        if (d > 0 && d <= 20) bid_c20 += 1.0f;
    ...
    int                        direction        = 0)
{
    ...
    for (int i = 0; i < 31 && r.contract[i]; ++i)
