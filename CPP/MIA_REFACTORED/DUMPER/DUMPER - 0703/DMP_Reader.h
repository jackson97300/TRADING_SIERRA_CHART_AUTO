...
inline void DMP_ReadBNSignals(SCStudyInterfaceRef sc, DMP_RawData& d) {
    ...
    // ── Signaux booléens — sg0 "Color Bar" per-bar via DMP_ReadBN_Trigger ──
    //  🆕 FIX 07/03/2026 : SumOfAlerts(sg2) → Trigger(sg0)
    //  sg2 = compteur cumulatif de session → constante 1 après 1er fire = INUTILISABLE
    //  sg0 = Color Bar = prix quand la condition fire, 0 sinon = PER-BAR correct
    //  SafeBool(prix) → 1 si fire, 0 sinon. Fonctionne même sans onglet Alerts.
    d.bn_color_up   = DMP_ReadBN_Trigger(sc, chart,
                        d.is_nq ? DMP_Studies::NQ_FP::COLOR_UP   : DMP_Studies::ES_FP::COLOR_UP);
    d.bn_color_dn   = DMP_ReadBN_Trigger(sc, chart,
    ...
    // ES = Double Ask/Bid — NQ = Triple Ask/Bid (même fix sg0)
    if (!d.is_nq) {
        d.bn_double_ask  = DMP_ReadBN_Trigger(sc, chart, DMP_Studies::ES_FP::DOUBLE_ASK);
        d.bn_double_bid  = DMP_ReadBN_Trigger(sc, chart, DMP_Studies::ES_FP::DOUBLE_BID);
        d.bn_triple_ask  = DMP_BOOL_FALSE;
        d.bn_triple_bid  = DMP_BOOL_FALSE;
    } else {
        d.bn_double_ask  = DMP_BOOL_FALSE;
        d.bn_double_bid  = DMP_BOOL_FALSE;
        d.bn_triple_ask  = DMP_ReadBN_Trigger(sc, chart, DMP_Studies::NQ_FP::TRIPLE_ASK);
        d.bn_triple_bid  = DMP_ReadBN_Trigger(sc, chart, DMP_Studies::NQ_FP::TRIPLE_BID);
    }

    // 🆕 BUG #10 — Volume UP/DOWN (NQ) + Edge Zones Footprint (05/03/2026)
    // Le Bot lit : bn.edge_buy/sell (sg2) + bn.volume_up/down (sg2, NQ only)
    if (!d.is_nq) {
        d.bn_volume_up   = DMP_BOOL_FALSE;  // ES n'a pas VOLUME UP/DOWN
        d.bn_volume_dn   = DMP_BOOL_FALSE;
...
inline void DMP_ReadBarSignals(SCStudyInterfaceRef sc, DMP_RawData& d) {
    ...
    // ── A. Signaux booléens (sg0 via DMP_ReadBN_Trigger) ─────────────────
    d.bar_color_up    = DMP_ReadBN_Trigger(sc, chart,
                        d.is_nq ? DMP_Studies::NQ_BARRES::COLOR_UP    : DMP_Studies::ES_BARRES::COLOR_UP);
    d.bar_color_dn    = DMP_ReadBN_Trigger(sc, chart,
    ...
    if (!d.is_nq) {
        d.bar_pressure_ask = DMP_ReadBN_Trigger(sc, chart, DMP_Studies::ES_BARRES::DOUBLE_ASK);
        d.bar_pressure_bid = DMP_ReadBN_Trigger(sc, chart, DMP_Studies::ES_BARRES::DOUBLE_BID);
    } else {
        d.bar_pressure_ask = DMP_ReadBN_Trigger(sc, chart, DMP_Studies::NQ_BARRES::TRIPLE_ASK);
        d.bar_pressure_bid = DMP_ReadBN_Trigger(sc, chart, DMP_Studies::NQ_BARRES::TRIPLE_BID);
    }

    // ── B. Extension Lines prix (sg1 via DMP_SafeReadLast) ───────────────
    const int SG_EXT = 1;  // Subgraph 1 = Extension Lines PRIX

    d.ext_color_up_px  = DMP_SafeReadLast(sc, chart,
                         d.is_nq ? DMP_Studies::NQ_BARRES::COLOR_UP    : DMP_Studies::ES_BARRES::COLOR_UP,    SG_EXT);
    d.ext_color_dn_px  = DMP_SafeReadLast(sc, chart,
                         d.is_nq ? DMP_Studies::NQ_BARRES::COLOR_DN    : DMP_Studies::ES_BARRES::COLOR_DN,    SG_EXT);
    d.ext_long_up_px   = DMP_SafeReadLast(sc, chart,
                         d.is_nq ? DMP_Studies::NQ_BARRES::LONG_UP_BAR : DMP_Studies::ES_BARRES::LONG_UP_BAR, SG_EXT);
    d.ext_long_dn_px   = DMP_SafeReadLast(sc, chart,
                         d.is_nq ? DMP_Studies::NQ_BARRES::LONG_DN_BAR : DMP_Studies::ES_BARRES::LONG_DN_BAR, SG_EXT);
    d.ext_edge_buy_px  = DMP_SafeReadLast(sc, chart,
                         d.is_nq ? DMP_Studies::NQ_BARRES::EDGE_BUY    : DMP_Studies::ES_BARRES::EDGE_BUY,    SG_EXT);
    d.ext_edge_sell_px = DMP_SafeReadLast(sc, chart,
                         d.is_nq ? DMP_Studies::NQ_BARRES::EDGE_SELL   : DMP_Studies::ES_BARRES::EDGE_SELL,   SG_EXT);
}
...
inline void DMP_ReadDaily(SCStudyInterfaceRef sc, DMP_RawData& d) {
    ...
    //  Migré de chart daily (16/17) vers charts barres (25/23) le 02/03/2026.
    //  Sur chart daily, toutes les bandes = même valeur (1 barre/jour → pas de SD).
    //  Sur chart intraday, VWAP W/M se calcule avec toutes les barres 1min → SD correctes.
    //  Charts BARRES = même timeframe que host → DMP_SafeRead (per-bar) fonctionne.
    const int chart_b = d.is_nq ? DMP_Charts::NQ_BARRES : DMP_Charts::ES_BARRES;

    const int vww_id = d.is_nq ? DMP_Studies::NQ_BARRES::VWAP_WEEKLY : DMP_Studies::ES_BARRES::VWAP_WEEKLY;
    d.vwap_weekly      = DMP_SafeRead(sc, chart_b, vww_id, 0);  // sg0 = VWAP
    d.vwap_weekly_sd1u = DMP_SafeRead(sc, chart_b, vww_id, 1);  // sg1 = Top Band 1 (SD+1)
    d.vwap_weekly_sd1d = DMP_SafeRead(sc, chart_b, vww_id, 2);  // sg2 = Bottom Band 1 (SD-1)

    const int vwm_id = d.is_nq ? DMP_Studies::NQ_BARRES::VWAP_MONTHLY : DMP_Studies::ES_BARRES::VWAP_MONTHLY;
    d.vwap_monthly      = DMP_SafeRead(sc, chart_b, vwm_id, 0);  // sg0 = VWAP
    d.vwap_monthly_sd1u = DMP_SafeRead(sc, chart_b, vwm_id, 1);  // sg1 = Top Band 1 (SD+1)
    d.vwap_monthly_sd1d = DMP_SafeRead(sc, chart_b, vwm_id, 2);  // sg2 = Bottom Band 1 (SD-1)
}
...
inline void DMP_ReadAll(SCStudyInterfaceRef sc, DMP_RawData& d, bool is_nq) {
    ...
    //
    //  NOTE: UTC-5 toute l'année (simplification identique à l'ancien dumper).
    //  Pour le DST: UTC-4 du 2ème dimanche de mars au 1er dimanche de nov.
    //  Mais la simplification UTC-5 décale juste les sessions de 1h pendant l'été,
    //  ce qui est acceptable (l'ancien dumper faisait pareil).
    {
        ...
        // UTC → ET (New York) : -5h (identique à l'ancien dumper)
