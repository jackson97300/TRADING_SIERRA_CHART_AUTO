...
static inline int DMP_FormatJSONL(const DMP_MLFeatures& f, char* buf) {
    ...
    DMP_WR_KV(buf, pos, "n_big_ask", f.n_big_ask);DMP_WR_COMMA(buf, pos);
    DMP_WR_KV(buf, pos, "n_big_bid", f.n_big_bid);DMP_WR_COMMA(buf, pos);
    ...
    DMP_WR_KV(buf, pos,  "bars_since_retest_low",  f.bars_since_retest_low);
