...
SCSFExport scsf_MIA_AutoTrader_BN(SCStudyInterfaceRef sc) {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // 🆕 14/03/2026: SESSION PLANNER — Diagnostic + Asia IB + No-trade zones
    // Tourne à chaque barre, mais BuildSessionPlan() seulement quand la phase change.
    // Prouvé par sim Python: PF 1.79, 20 trades NQ+ES.
    // ═══════════════════════════════════════════════════════════════════════════
    {
        int now_min_et = GetMinutesSinceMidnightET(sc.CurrentSystemDateTime);
        SessionPhase current_phase = DetectPhase(now_min_et);

        // ── Asia IB: tracker le range pendant 20h-22h ET ──
        if (current_phase == PHASE_ASIA_IB) {
            // Utiliser le prix le plus récent (ES pour les deux, le range est partagé)
            float bar_high = (current_price_es > 0) ? current_price_es : current_price_nq;
            float bar_low = bar_high;  // Approximation — on n'a pas le high/low ici
            UpdateAsiaIB(g_asia_ib, bar_high, bar_low, atr_nq, CONFIG_NQ.tick_size);
        }

        // ── Vérifier cassures IB à chaque barre ──
        if (current_price_es > 0) CheckIBBreak(g_asia_ib, current_price_es);
        if (current_price_nq > 0) CheckIBBreak(g_asia_ib, current_price_nq);

        // ── Recalculer le plan quand la phase change (ES) ──
        if (current_phase != g_last_phase_es && current_price_es > 0) {
            g_session_plan_es = BuildSessionPlan(sc,
                current_price_es, CONFIG_ES.tick_size, atr_es,
                // Profile (approximations — sera affiné avec les données VP)
                0.5f, false,     // poc_ratio, has_double_dist (TODO: lire depuis VP)
                0, 0.0f,         // open_type, gap_ticks (TODO: lire depuis données)
                // Levels (lire depuis MQ et données en cache)
                mq_es.vwap, 0, 0, mq_es.vwap,   // prev_vpoc/vah/val/vwap (TODO: remplir)
                0, 0, 0,                           // cur_vpoc/vah/val (TODO: remplir depuis VP)
                mq_es.hvl_price, 0, 0,             // mq_hvl, put_0dte, call_0dte
                mq_es.gex_up[0], mq_es.gex_down[0],  // gex_up, gex_dn
                0, 0, 0,                           // ovn_high, ovn_low, open_cash
                g_asia_ib, current_phase);

            // Reset per-session trade count si nouvelle session
            if (current_phase == PHASE_LONDON_ACTIVE)  g_trades_london_es = 0;
            if (current_phase == PHASE_US_ACTIVE)      g_trades_us_es = 0;

            g_last_phase_es = current_phase;
            g_session_plan_es.log(sc);
        }

        // ── Recalculer le plan NQ ──
        if (current_phase != g_last_phase_nq && current_price_nq > 0) {
            g_session_plan_nq = BuildSessionPlan(sc,
                current_price_nq, CONFIG_NQ.tick_size, atr_nq,
                0.5f, false,
                0, 0.0f,
                mq_nq.vwap, 0, 0, mq_nq.vwap,
                0, 0, 0,
                mq_nq.hvl_price, 0, 0,
                mq_nq.gex_up[0], mq_nq.gex_down[0],
                0, 0, 0,
                g_asia_ib, current_phase);

            if (current_phase == PHASE_LONDON_ACTIVE)  g_trades_london_nq = 0;
            if (current_phase == PHASE_US_ACTIVE)      g_trades_us_nq = 0;

            g_last_phase_nq = current_phase;
            g_session_plan_nq.log(sc);
        }

        // ── Intra-session update (chaque barre) ──
        if (g_session_plan_es.plan_valid && current_price_es > 0) {
            UpdatePlan(g_session_plan_es, current_price_es, false, 0.0f, CONFIG_ES.tick_size);
        }
        if (g_session_plan_nq.plan_valid && current_price_nq > 0) {
            UpdatePlan(g_session_plan_nq, current_price_nq, false, 0.0f, CONFIG_NQ.tick_size);
        }

        // ── Stocker la phase dans le dashboard ──
        strncpy(g_dashboard.current_session, GetCurrentSessionName(sc), sizeof(g_dashboard.current_session) - 1);
    }
    ...
    if (es_state_local.circuit_breaker_active) {
        ...
        // 🆕 14/03/2026: SESSION PLANNER — No-trade zones + per-session limit
        if (can_trade && g_session_plan_es.plan_valid) {
            int now_et = GetMinutesSinceMidnightET(sc.CurrentSystemDateTime);
            
            // No-trade zone (London transition, US transition)
            if (g_session_plan_es.is_no_trade(now_et)) {
                can_trade = false;
                strcpy(g_dashboard.bot_action_es, "NO_TRADE_ZONE");
                strcpy(g_dashboard.no_trade_reason_es, "Zone de transition — pas de nouveau trade");
            }
            
            // Phase non autorisée (Asia, Pre_London)
            SessionPhase phase_es = DetectPhase(now_et);
            if (can_trade && !g_session_plan_es.is_trade_phase(phase_es)) {
                can_trade = false;
                strcpy(g_dashboard.bot_action_es, "PHASE_OFF");
                strcpy(g_dashboard.no_trade_reason_es, "Phase non autorisee par le Session Planner");
            }
            
            // Per-session trade limit (max 5 par session — prouvé par sim)
            if (can_trade) {
                int session_trades_es = 0;
                if (phase_es == PHASE_LONDON_ACTIVE) session_trades_es = g_trades_london_es;
                else if (phase_es >= PHASE_US_IB_FORMING) session_trades_es = g_trades_us_es;
                
                if (session_trades_es >= g_session_plan_es.max_trades_session) {
                    can_trade = false;
                    strcpy(g_dashboard.bot_action_es, "SESSION_MAX");
                    snprintf(g_dashboard.no_trade_reason_es, sizeof(g_dashboard.no_trade_reason_es),
                             "Max %d trades atteint pour cette session", g_session_plan_es.max_trades_session);
                }
            }
        }
                    ...
                    if (l3.veto) {
                        ...
                        if (!l4.passed) {
                            ...
                            if (order_sent) {
                                ...
                                // 🆕 14/03/2026: Incrémenter compteur per-session
                                {
                                    int now_et_trade = GetMinutesSinceMidnightET(sc.CurrentSystemDateTime);
                                    SessionPhase ph = DetectPhase(now_et_trade);
                                    if (ph == PHASE_LONDON_ACTIVE) g_trades_london_es++;
                                    else if (ph >= PHASE_US_IB_FORMING) g_trades_us_es++;
                                }
                            } else {
    ...
    if (nq_state_local.circuit_breaker_active) {
        ...
        // 🆕 14/03/2026: SESSION PLANNER — No-trade zones + per-session limit (NQ)
        if (can_trade_nq && g_session_plan_nq.plan_valid) {
            int now_et_nq = GetMinutesSinceMidnightET(sc.CurrentSystemDateTime);
            
            if (g_session_plan_nq.is_no_trade(now_et_nq)) {
                can_trade_nq = false;
                strcpy(g_dashboard.bot_action_nq, "NO_TRADE_ZONE");
                strcpy(g_dashboard.no_trade_reason_nq, "Zone de transition — pas de nouveau trade");
            }
            
            SessionPhase phase_nq = DetectPhase(now_et_nq);
            if (can_trade_nq && !g_session_plan_nq.is_trade_phase(phase_nq)) {
                can_trade_nq = false;
                strcpy(g_dashboard.bot_action_nq, "PHASE_OFF");
                strcpy(g_dashboard.no_trade_reason_nq, "Phase non autorisee par le Session Planner");
            }
            
            if (can_trade_nq) {
                int session_trades_nq = 0;
                if (phase_nq == PHASE_LONDON_ACTIVE) session_trades_nq = g_trades_london_nq;
                else if (phase_nq >= PHASE_US_IB_FORMING) session_trades_nq = g_trades_us_nq;
                
                if (session_trades_nq >= g_session_plan_nq.max_trades_session) {
                    can_trade_nq = false;
                    strcpy(g_dashboard.bot_action_nq, "SESSION_MAX");
                    snprintf(g_dashboard.no_trade_reason_nq, sizeof(g_dashboard.no_trade_reason_nq),
                             "Max %d trades atteint pour cette session", g_session_plan_nq.max_trades_session);
                }
            }
        }
                    ...
                    if (l3_nq.veto) {
                        ...
                        if (!l4_nq.passed) {
                            ...
                            if (order_sent_nq) {
                                ...
                                // 🆕 14/03/2026: Incrémenter compteur per-session NQ
                                {
                                    int now_et_trade_nq = GetMinutesSinceMidnightET(sc.CurrentSystemDateTime);
                                    SessionPhase ph_nq = DetectPhase(now_et_trade_nq);
                                    if (ph_nq == PHASE_LONDON_ACTIVE) g_trades_london_nq++;
                                    else if (ph_nq >= PHASE_US_IB_FORMING) g_trades_us_nq++;
                                }
